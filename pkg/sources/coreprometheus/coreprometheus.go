...
func (src *prometheusMetricsSource) Collect(ctx context.Context) (*sources.MetricsBatch, error) {
	stats, err := src.kubeletClient.GetCorePrometheus(ctx, src.node.ConnectAddress)
	...
	for _, family := range stats {
		...
		for _, metric := range family.GetMetric() {
			...
			value := resource.NewQuantity(int64(metric.GetGauge().GetValue()), resource.DecimalSI)
...
func (src *prometheusMetricsSource) decodePodStats(name string, value *resource.Quantity, timestampMs int64, info *labelInfo, targets *[]sources.PodMetricsPoint) {
...
func decodeMetric(name string, value *resource.Quantity, timestampMs int64, target *sources.MetricsPoint) {
	if target == nil {
	...
	case "cpu_usage_seconds_total":
		target.CpuUsage = *value
	case "memory_working_set_bytes":
		target.MemoryUsage = *value
	case "ephemeral_storage_usage_bytes":
		target.EphemeralStorageUsage = *value
	}
}
