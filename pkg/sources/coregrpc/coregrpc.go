...
type grpcMetricsSource struct {
	node          NodeInfo
	kubeletClient KubeletCoreInterface
}

func NewCoreMetricsSource(node NodeInfo, client KubeletCoreInterface) sources.MetricSource {
...
func (src *grpcMetricsSource) Collect(ctx context.Context) (*sources.MetricsBatch, error) {
	stats, err := src.kubeletClient.GetCoreGRPC(ctx, src.node.ConnectAddress)
...
func (src *grpcMetricsSource) decodeNodeStats(stats *corestats.Usage, target *sources.NodeMetricsPoint) error {
	...
	*target = sources.NodeMetricsPoint{
		Name:         src.node.Name,
		MetricsPoint: statsToMetricsPoint(stats),
	}
	return nil
}
	...
	*target = sources.PodMetricsPoint{
	...
	for i, container := range stats.Containers {
		target.Containers[i] = sources.ContainerMetricsPoint{
			Name:         container.Name,
			MetricsPoint: statsToMetricsPoint(container.Usage),
		}
	}
	return nil
}

func statsToMetricsPoint(stats *corestats.Usage) sources.MetricsPoint {
	return sources.MetricsPoint{
		...
		EphemeralStorageUsage: *uint64Quantity(stats.EphemeralStorageUsageBytes, 0),
	}
}
...
func NewCoreGRPCProvider(nodeLister v1listers.NodeLister, kubeletClient KubeletCoreInterface, addrResolver summary.NodeAddressResolver) sources.MetricSourceProvider {
